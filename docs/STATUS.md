...
| **M8** demo shell | ✅ | a thin **browser-demo onboarding** layer (not Wolf3D title chrome): a **thesis/start card** (slice 1) — "Wolf3D simulated by an EVM contract; the chain is the netcode" + the control list — shown before boot (the Engine/Map/Session deploy runs behind it so the click lands on a ready game), doubling as the **audio-unlock** gesture the AudioContext already needs; a **restart / "New game"** control (slice 2, button + Enter) that redeploys a fresh `Session` against the same immutable engine+map (no page reload, no asset/wasm re-fetch) and resets client state — `main()` split into one-time setup + a re-invokable `runSession()`, an old session's loops cancelled by a generation token, render state (tiles→base + renderer re-sync, death/level-end/fx/tick) reset on each entry; and on-screen **controls help** (in the card). Both slices verified in-browser. **Descoped from the original "presentation shell"** (title / menu / "Get Psyched!" / episode flow): one immutable Map per Session ⟹ nothing to select and no level→level flow, the browser tab is already the title, and the **level-intermission tally already shipped** (M6 slice 4 — the level-complete sequence) |
...
- **M8 ✅ done** — demo shell: a thesis/start card (slice 1 — what this is + controls, doubling as the
  audio-unlock gesture; the stack deploys behind it) and a restart / "New game" control (slice 2 — button
  + Enter, redeploys a fresh `Session` on the same immutable engine+map and resets client state, so retry
  isn't a page reload), both verified in-browser. The original "presentation shell" (title / menu /
  "Get Psyched!" / episode flow) stays **descoped** — one immutable Map per Session ⟹ no menu/episode flow,
