...
a PvE loop simulated entirely in a Solidity contract, **verified bit-for-bit against the original id
C code**, at ~65–100k gas per input (one guard + door), with a live **first-person** browser view
(raycaster + HUD + minimap), all decoded from on-chain state.
